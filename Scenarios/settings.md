...
| <a id="enableCodeCoverage"></a>enableCodeCoverage | PREVIEW: When set to true, code coverage is collected while running normal tests and exported to a Cobertura XML report. Code coverage is only produced by the separate RunTests action, so this setting requires [useSeparateTestAction](#useSeparateTestAction) to be enabled; it has no effect otherwise. | false |
| <a id="codeCoverageSetup"></a>codeCoverageSetup | PREVIEW: Configuration for code coverage collection (used when [enableCodeCoverage](#enableCodeCoverage) is set). This is a structure with the following properties:<br />**trackingType** = the granularity of coverage tracking (PerRun, PerCodeunit or PerTest). Default is PerRun.<br />**produceCodeCoverageMap** = the granularity of the coverage map output (Disabled, PerCodeunit or PerTest). Default is PerCodeunit.<br />**excludeFilesPattern** = an array of glob patterns for source files to exclude from the coverage report. Default is empty.<br />**filterToRepoObjectIds** = when true, the External Objects section of the coverage report only lists objects whose ID falls within the repo apps' declared id ranges (from app.json), hiding Microsoft/system objects. Default is true. | See description |
